...
// @desc    Upload a file for message
// @route   POST /api/messages/upload
// @access  Private
exports.uploadFile = asyncHandler(async (req, res, next) => {
    // Support both single-field (`req.file`) and any-field (`req.files`) multer usage
    const incomingFile = req.file || (Array.isArray(req.files) && req.files.length > 0 && req.files[0]);
    if (!incomingFile) {
        console.log('[uploadFile] No file found on request. headers:', req.headers['content-type']);
    ...
    const filename = `${Date.now()}_${incomingFile.originalname.replace(/\s+/g, '_')}`;
    const filepath = path.join(filesDir, filename);
    fs.writeFileSync(filepath, incomingFile.buffer);

    try {
        console.log(`[uploadFile] Saved ${incomingFile.originalname} (${incomingFile.size} bytes) -> ${filepath}`);
    } catch (e) {}

    const publicUrl = `/uploads/files/${filename}`;

    res.status(200).json({ success: true, data: { fileUrl: publicUrl, fileName: incomingFile.originalname, fileSize: incomingFile.size, fileType: incomingFile.mimetype } });
});